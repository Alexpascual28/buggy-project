...
void orientation(String m){
  if(m.equals("front") == true){
    ...
    motor1("CW");
    motor2("OFF");
  } else if(m.equals("rightfast") == true){
    motor1("CCW");
    motor2("CCW");
  } else if(m.equals("leftfast") == true){
    motor1("CW");
    motor2("CW");
  } else {
    motor1("OFF");
...
void motion(float k, float j){
  velocity = 255*sqrt(pow(k, 2)+(pow(j, 2)));
  ...
  if(k>=0&&j>=0){
    ...
    if(k>j){
       ...
       orientation("leftfast");
    }
  } else if(k>=0&&j<0){
    if(k>abs(j)){
       orientation("rightfast");
    } else if(abs(j)>=k){
       orientation("back");
    }
  }
